...
func (n *Node1) broadcastReleaseMsg(broadcastMsg string, no_client int) { //holds releaseCounter to check if experiment has terminated.
	for i := 0; i < len(n.directoryOfNodes); i++ {
		// fmt.Printf("client %v broadcasting. check don: %v\n", i, n.directoryOfNodes)
...
func (n *Node1) requestEnterCriticalSection(no_client int) {
	...
	// fmt.Printf("ur checking %v", n.repliesFromOtherMachines)
	...
	fmt.Printf("hello checkIfAllReplyReceivedClientBroadcast:%v ", n.checkIfAllReplyReceivedClientBroadcast())
	for {
		...
		if n.checkIfAllReplyReceivedClientBroadcast() == true {
			...
			go n.broadcastReleaseMsg(releaseMsg, no_client)
...
//////////////////////////////////
func (n *Node1) clientTask1(no_client int) {
	// request once to enter critical section
	go n.requestEnterCriticalSection(no_client)
	for {
