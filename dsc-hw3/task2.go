...
func (c *CentralManager) returnIfHigherId(incoming_node_id int) bool {
	if c.coordinator_id > incoming_node_id {
		return true
	} else {
		return false
	}
...
func (c *CentralManager) broadcastDataCM(kill_channel chan bool) {
	for {
		...
		case msg2 := <-kill_channel:
			if msg2 == false {
				//kill broadcast data
				fmt.Printf("broadcasting for CM %v was killed", c.id)
				return
			}
...
func (c *CentralManager) callElection() {
	...
	if !(c.election_stage == "election") {
		...
		if c.alive == true {
			for k, _ := range c.election_directory_of_nodes {
		...
		for {
			if int(time.Now().Unix()-waitRejectMessage) > REJECT_ELECTION_TIME_LIMIT || c.alive == false {
				break
			}
		}
		// STAGE 3: BROADCAST
		...
		case msg1 := <-c.election_directory_of_nodes[c.id]:
			...
			if c.alive == false {
				...
				if strings.Contains(msg1, "CMR") { // this is a message from primary replica to update the secondary Central manager replicas
					fmt.Printf("Client %v has a message from primary replica to update the secondary Central manager replicas\n", c.id)
					msgString := strings.Split(msg1, "&")
					cmr := changeMessageIntoCentralManagerRecord(msgString[1])
					c.data = cmr
					fmt.Printf("Client %v 's struct for data now: %v\n", c.id, c.data)
				}
			}
		case msg2 := <-c.control_panel_for_alive_node[c.id]:
